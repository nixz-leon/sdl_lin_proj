...
Circ::Circ(std::string new_name, int new_color[3], int new_r, float new_p[2], float new_v[2], float new_a[2])
{
	name = new_name;
	color[0] = new_color[0];
	color[1] = new_color[1];
...
void collisionevent(Circ& a, Circ& b, float timestep)
{
	...
	if (squaredsize <= mindistance) {
		...
		tempa[0] = a.position[0];
		tempa[1] = a.position[1];
		tempb[0] = b.position[0];
		tempb[1] = b.position[1];
		sub(tempa, midpoint);
		sub(tempb, midpoint);
		normalize(tempa);
		normalize(tempb);
